...
from protein_mpnn.features.build_features import CA_ProteinFeatures, ProteinFeatures, tied_featurize
from protein_mpnn.features.utils import (
    DecLayer,
    EncLayer,
    cat_neighbors_nodes,
    gather_nodes,
...
from protein_mpnn.models.utils import (
...
class ProteinMPNN(nn.Module):
    ...
    def sample(
        ...
        for t_ in range(N_nodes):
            ...
            bias_by_res_gathered = torch.gather(bias_by_res, 1, t[:, None, None].repeat(1, 1, 21))[
                :, 0, :
            ]  # [B, 21]
            if (mask_gathered == 0).all():  # for padded or missing regions only
                ...
                for idx, layer in enumerate(self.decoder_layers):
                    ...
                    h_ESV_decoder_t = cat_neighbors_nodes(h_V_stack[idx], h_ES_t, E_idx_t)
                    h_V_t = torch.gather(
                        h_V_stack[idx],
                        1,
                        t[:, None, None].repeat(1, 1, h_V_stack[idx].shape[-1]),
                    )
                    h_ESV_t = (
                        torch.gather(
                            mask_bw,
                ...
                if pssm_bias_flag:
                    ...
                    ) * probs + pssm_multi * pssm_coef_gathered[:, None] * pssm_bias_gathered
    ...
    def tied_sample(
        ...
        decoding_order = torch.tensor(list(itertools.chain(*new_decoding_order)), device=device)[
            None,
        ].repeat(X.shape[0], 1)
        ...
        for t_list in new_decoding_order:
            ...
            for t in t_list:
                if (mask[:, t] == 0).all():
                    ...
                    for idx, layer in enumerate(self.decoder_layers):
                        h_ESV_decoder_t = cat_neighbors_nodes(h_V_stack[idx], h_ES_t, E_idx_t)
                        h_V_t = h_V_stack[idx][:, t : t + 1, :]
                        h_ESV_t = mask_bw[:, t : t + 1, :, :] * h_ESV_decoder_t + h_EXV_encoder_t
                        h_V_stack[idx + 1][:, t, :] = layer(h_V_t, h_ESV_t, mask_V=mask_t).squeeze(
                            1
                        )
                    h_V_t = h_V_stack[-1][:, t, :]
            ...
            if done_flag:
                ...
                S_t_repeat = (
                    chain_mask[:, t] * S_t_repeat + (1 - chain_mask[:, t]) * S_true[:, t]
